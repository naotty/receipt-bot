...
メール本文から商品名、金額、支払い方法、適切な勘定科目を抽出してください。商品名がない場合は、サービス名を商品名としてください。以下のJSON形式で返してください。後続の処理で使うので絶対にコードブロックにはしないでください。
...
${content || '(本文なし。添付画像のみを優先して抽出してください)'}`;
  ...
  try {
    ...
    if (isBedrockMarketplaceAccessDeniedError(error)) {
      console.error(
        `Bedrockモデル ${modelId} の利用権限が不足しています。` +
        'Lambda実行ロールに aws-marketplace:ViewSubscriptions / aws-marketplace:Subscribe を付与し、' +
        '対象モデルのModel access（またはMarketplaceサブスクリプション）を有効化してください。設定反映後は2分ほど待って再実行してください。'
      );
    }
    throw error;
  }
...
export function parseAndValidateExtractedData(extractedText: string): ExtractedData {
  const normalizedJsonText = normalizeModelJsonOutput(extractedText);
  const raw = JSON.parse(normalizedJsonText) as {
    items?: Array<{
      name?: unknown;
      amount?: unknown;
...
function isBedrockMarketplaceAccessDeniedError(error: unknown): boolean {
  ...
  return name === 'AccessDeniedException' &&
    message.includes('aws-marketplace:ViewSubscriptions') &&
    message.includes('aws-marketplace:Subscribe');
}
